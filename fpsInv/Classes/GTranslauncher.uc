...
function float GetAIRating()
{
	...
	if (B.bTranslocatorHop && ((B.Focus == B.MoveTarget) || ((B.TranslocationTarget != None) && (B.Focus == B.TranslocationTarget))) && B.Squad.AllowTranslocationBy(B))
	{
		if ((GTransBeacon != None) && GTransBeacon.IsMonitoring(GTransbeacon.TranslocationTarget))
		{
			...
			if  ((GTransbeacon.TranslocationTarget == Instigator.Controller.MoveTarget)
...
{
	if ((GTransBeacon == None) || (Instigator.Controller.IsA('PlayerController') && (PlayerController(Instigator.Controller).ViewTarget == GTransBeacon)))
		ViewPlayer();
	else
		ViewCamera();
}
...
simulated event RenderOverlays( Canvas Canvas )
{
	...
	if ((PlayerController(Instigator.Controller) != None) && (PlayerController(Instigator.Controller).ViewTarget == GTransBeacon))
	{
		...
		Canvas.Style = 255;
		Canvas.SetPos(0,0);
		Canvas.DrawTile(Material'TransCamFB', Canvas.SizeX, Canvas.SizeY, 0.0, 0.0, 512, 512);
		Canvas.SetPos(0,0);
...
defaultproperties
{
     ...
     FireModeClass(0)=Class'fpsInv.GTransFire'
     FireModeClass(1)=Class'fpsInv.GTransRecall'
     PutDownAnim="PutDown"
     ...
     InventoryGroup=10
     PickupClass=Class'fpsInv.GTransPickup'
     ...
     Skins(1)=Texture'fpsInv.GoldTransTex' //'WeaponSkins.Skins.NEWTranslocatorTEX'
