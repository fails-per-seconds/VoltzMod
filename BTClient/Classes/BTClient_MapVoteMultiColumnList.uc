...
function DrawItem(Canvas C, int i, float X, float Y, float W, float H, bool bSelected, bool bPending)
{
	...
	DrawStyle.DrawText(C, MenuState, CellLeft, Y, CellWidth, H, TXTA_Left, string(int(float(VRI.MapList[MapVoteData[SortData[i].SortItem]].PlayCount)/100F))$"h", FontScale);

	GetCellLeftWidth(1, CellLeft, CellWidth);
	DrawStyle.FontColors[0] = GetColumnColor(1);
	DrawStyle.DrawText(C, MenuState, CellLeft, Y, CellWidth, H, TXTA_Left, mapTxt, FontScale);


	GetCellLeftWidth(2, CellLeft, CellWidth);
	DrawStyle.FontColors[0] = GetColumnColor(2);
	DrawStyle.DrawText(C, MenuState, CellLeft, Y, CellWidth, H, TXTA_Left, recordsCountTxt, FontScale);

	GetCellLeftWidth(3, CellLeft, CellWidth );
	DrawStyle.FontColors[0] = GetColumnColor(3);
	DrawStyle.DrawText(C, MenuState, CellLeft, Y, CellWidth, H, TXTA_Left, timeTxt, FontScale);

	GetCellLeftWidth(4, CellLeft, CellWidth);
	DrawStyle.FontColors[0] = GetColumnColor(4);
	DrawStyle.DrawText(C, MenuState, CellLeft, Y, CellWidth, H, TXTA_Left, holderTxt, FontScale);

	GetCellLeftWidth(5, CellLeft, CellWidth);
	DrawStyle.FontColors[0] = GetColumnColor(5);
	DrawStyle.DrawText(C, MenuState, CellLeft, Y, CellWidth, H, TXTA_Left, mapRatingTxt, FontScale);

	GetCellLeftWidth(6, CellLeft, CellWidth);
	DrawStyle.FontColors[0] = GetColumnColor(6);
	DrawStyle.DrawText(C, MenuState, CellLeft, Y, CellWidth, H, TXTA_Right, string(VRI.MapList[MapVoteData[SortData[i].SortItem]].Sequence), FontScale);
...
defaultproperties
{
     OnFilterVotingList=BTClient_MapVoteMultiColumnList.InternalOnFilterVotingList
     ColumnHeadings(0)="Hours"
     ColumnHeadings(1)="Map Name"
     //ColumnHeadings(2)="Records"
     //ColumnHeadings(3)="Time"
     //ColumnHeadings(4)="Set By"
     ColumnHeadings(2)="Thumbs"
     ColumnHeadings(3)="Seq"
     InitColumnPerc(0)=0.065000
     InitColumnPerc(1)=0.375000
     //InitColumnPerc(2)=0.085000
     //InitColumnPerc(3)=0.115000
     //InitColumnPerc(4)=0.180000
     InitColumnPerc(2)=0.080000
     ...
     ColumnHeadingHints(2)="Players rating of this map."
     ColumnHeadingHints(3)="Sequence, The number of games that have been played since this map was last played."
     GetItemHeight=BTClient_MapVoteMultiColumnList.InternalGetItemHeight
