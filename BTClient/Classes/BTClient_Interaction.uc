...
var private const string RecordTimeMsg, RecordHolderMsg, RecordTimeLeftMsg, RecordEmptyMsg, RecordHubMsg, RecordTimeElapsed, RankingHideMsg;
var private string RankingKeyMsg, OldKey;

var private BTClient_MutatorReplicationInfo MRI;
var private HUD myHUD;
var private BTClient_Config BTConfig;
var private BTClient_ClientReplication SpectatedClient;
var private BTClient_LevelReplication ActiveLevel;

var private array<Pickup> KeyPickupsList;

var private Color TimePositiveColor, TimeNegativeColor, TimeCheckColor, TimeTickColor;
var private bool bTestRun, bPauseTest, bMenuModified, bRenderAll, bRenderOnlyDynamic;

var private name RenderTag;
var private byte RenderMode;

var private int ElapsedTime;
var private float LastTickTime, Delay, YOffsetScale, LastTime, DrawnTimer;
...
exec function ShowBestTimes()
{
	...
	if (FPage != "")
	{
		...
		SendConsoleMessage("Sorry ShowBestTimes is not available on this server!");
}
...
exec function Player(string playerName)
{
...
exec function Map(string mapName)
{
	ShowMapInfo(mapName);
}
...
exec function RevoteMap()
{
	if (ViewportOwner.Actor.PlayerReplicationInfo.bAdmin || ViewportOwner.Actor.Level.NetMode == NM_StandAlone)
		...
		BT("VoteMap" @ Left(string(MRI), InStr(string(MRI), ".")));
}
...
event Tick(float DeltaTime)
{
	...
	{
		SendConsoleMessage("ShowAll is not allowed on this server, therefore you have been kicked");
...
private function PerformDodgePerk()
{
	...
	p.GetAnimParams(0, anim, animFrame, animRate);
...
private function ModifyMenu()
{
	...
	if (Menu != none)
	{
		...
		if (myMenu != none)
		{
			myMenu.MyButton.StyleName = "StoreButton"; //"AdvancedButton"
			myMenu.MyButton.Style = Menu.c_Main.Controller.GetStyle("StoreButton", myMenu.FontScale);  //"AdvancedButton"
...
private function RenderTitle(Canvas C, PlayerController player)
{
	...
	{
		if (P == player.ViewTarget || P.IsA('Monster') || P.bHidden || P.bDeleteMe || P.bDeRes)
			continue;
...
final function Vector DrawElement(Canvas C, float x, float y, string title, optional coerce string value, optional bool bCenter,
	optional float minWidth, optional float scaling, optional Color textColor, optional Color tileColor)
{
...
function PostRender(Canvas C)
{
	...
	local float YP, XP, XP1;
	local PlayerController player;

	player = ViewportOwner.Actor;
	if (player.myHUD.bShowScoreBoard || player.myHUD.bHideHUD || MRI == None || player.PlayerReplicationInfo == None)
		return;
...
//			RenderZoneActors(C, player);
	...
	if (MRI.RecordState == RS_Active)
	{
		...
		{
			bTimerPaused = player.IsInState('Dead') || player.ViewTarget == none || (xPawn(player.ViewTarget) != none && (xPawn(player.ViewTarget).bPlayedDeath || xPawn(player.ViewTarget).IsInState('Dying')));
		}
...
/*
	if (ActiveLevel != none && ActiveLevel.NumRecords > 0)
	{
		// Draw Level and percent
		S = "Rank";
		drawY += DrawElement(C, drawX, drawY, s, Eval(SpectatedClient.SoloRank == 0, "?", SpectatedClient.SoloRank)$"/"$ActiveLevel.NumRecords).y*1.2;
	}

	// draw btimes level and percent
	s = "BT Level";
...
*/
	// draw level
	s = Decimal(StatsInv.Data.Level);
	drawY += DrawElement(C, C.ClipX*1.0 - C.ClipX*0.095, drawY - C.ClipY*0.1, "Level", s,, 175, 0.5, class'HUD'.default.GoldColor).Y*1.2;
	//DrawElement(C, drawX, drawY, "Level", s,, 150, 0.5, class'HUD'.default.GoldColor);

	//draw exp/needexp
	SpectatedClient.BTExperience = StatsInv.Data.Experience;
	SpectatedClient.BTLevel = StatsInv.Data.NeededExp;
	xp	= Decimal(StatsInv.Data.Experience);
	xpn	= Decimal(StatsInv.Data.NeededExp);
	s	= "/";
	v = DrawElement(C, C.ClipX*1.0 - C.ClipX*0.095, drawY - C.ClipY*0.1, xp, s$xpn,, 175, 0.5);
	if (SpectatedClient.BTExperience > SpectatedClient.LastRenderedBTExperience)
	{
		SpectatedClient.BTExperienceChangeTime = SpectatedClient.Level.TimeSeconds;
		if (SpectatedClient.BTLevel < SpectatedClient.LastRenderedBTLevel)
			...
			SpectatedClient.BTExperienceDiff = (SpectatedClient.BTExperience / SpectatedClient.BTLevel) * 100f;
	}
	else if (SpectatedClient.BTLevel > SpectatedClient.LastRenderedBTLevel)
	{
		SpectatedClient.BTExperienceChangeTime = SpectatedClient.Level.TimeSeconds;
	...
	C.DrawColor = #0x00AA0088;
	DrawLayer(C, C.ClipX*1.0 - C.ClipX*0.095, drawY - C.ClipY*0.1, v.x*(SpectatedClient.BTExperience/SpectatedClient.BTLevel), v.y);
	SpectatedClient.LastRenderedBTExperience = SpectatedClient.BTExperience;
	SpectatedClient.LastRenderedBTLevel = SpectatedClient.BTLevel;

	if (SpectatedClient.Level.TimeSeconds - SpectatedClient.BTExperienceChangeTime <= 1.5f && SpectatedClient.BTExperienceDiff != 0f)
	{
		C.SetPos((drawX + C.ClipX*0.835) + v.x, (drawY - C.ClipY*0.1) + v.y*1.2);
		if (SpectatedClient.BTExperienceDiff > 0)
		{
			C.DrawColor = Class'HUD'.default.GreenColor;
			C.DrawText("+ " $ SpectatedClient.BTExperienceDiff $ "%");
	...
	if (Pawn(ViewportOwner.Actor.ViewTarget) != none)
	{
		...
		v.Z = 0;
		s = Decimal(VSize(v));
		drawY += DrawElement(C, C.ClipX*0.5, drawY, "Speed", s, true, 100, 0.5, class'HUD'.default.GoldColor).Y*1.2;
...
private function float GetTimeLeft()
{
	if (MRI.bSoloMap)
	{
		if (BTConfig.bBaseTimeLeftOnPersonal && SpectatedClient.PersonalTime > 0.f)
			...
			return GetTopTime() - (MRI.Level.TimeSeconds - SpectatedClient.LastSpawnTime);
	}
	else
		return GetTopTime() - (MRI.Level.TimeSeconds - (MRI.MatchStartTime - MRI.CR.ClientMatchStartTime));
}
...
//Formats the given time in seconds.deci_centi. Outputs:[-][00:00:]00.00
static final function string FormatTime(float time, optional bool forceFull)
{
	...
	if (seconds < 10)
		secondString = "0" $ seconds; else secondString = string(seconds);
	if (minutes < 10)
		minuteString = "0" $ minutes; else minuteString = string(minutes);
	if (hours < 10)
		hourString = "0" $ hours; else hourString = string(hours);
...
//Formats the given time in seconds.deci_centi. Outputs:[-][00:00:]00.00 but only if the units are greater than null!
...
defaultproperties
{
     ...
     TimeTickColor=(G=255,R=255,A=255)
     YOffsetScale=0.600000
     ...
     Begin Object Class=TexRotator Name=oMarkerArrow
         ...
         VOffset=64
     End Object
     MarkerArrow=TexRotator'BTClient_Interaction.oMarkerArrow'
}
