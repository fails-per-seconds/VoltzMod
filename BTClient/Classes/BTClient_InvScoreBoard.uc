...
var() const string Header_LVL, Header_EXP, Header_CLS;
var protected float LX, LXL, EX, EXL, CX, CXL;
...
simulated function UpdateScoreBoard(Canvas C)
{
	...
	HX += 8+DXL;
	LX = HX;
	C.StrLen("0000000000", LXL, YL);

	// Calc RPGExp
	HX += 8+LXL;
	EX = HX;
	C.StrLen("000000000000000000", EXL, YL);

	// Calc RPGClass
	HX += 8+EXL;
	CX = HX;
	C.StrLen("000000000000000000", CXL, YL);
	//C.StrLen("0000000000", CXL, YL);
	...
	// Draw RPGXPGained SubHeader
	C.SetPos(EX, TY+YL+2);
	C.DrawColor = SecondaryColor;
	C.DrawText("XP Gained");

	// Draw RPGClass Header
	C.SetPos(CX, TY);
 	BTInterClass.static.DrawHeaderTile(C, CX, TY, CXL+4, YL);
	BTInterClass.static.DrawHeaderText(C, CX, TY, Header_CLS);
	// Draw RPGSubClass SubHeader
	C.SetPos(CX, TY+YL+2);
	C.DrawColor = SecondaryColor;
	C.DrawText("Mastery");
...
protected function RenderPlayerRow(Canvas C, PlayerReplicationInfo player, float x, float y, float rowWidth, float rowHeight)
{
	...
	local int EXP, L, NeedXP, XPGain, PClass;
	local string SClass;

	XPGain = -1;
	for(i = 0; i < CopyXPs.length; i++)
	{
		if (CopyXPs[i].PlayerName == player.PlayerName)
		{
			PClass = CopyXPs[i].PlayerClass;
			SClass = CopyXPs[i].SubClass;
			L = CopyXPs[i].InitialLV;
			EXP = CopyXPs[i].CurrentXP;
			NeedXP = CopyXPs[i].NeededXP;
			XPGain = CopyXPs[i].XPGained;
		}
	}
	...
	if (myClient != None && myClient.XPsUpdated)
	{
		...
		for(i = 0; i < myClient.CurrentXPs.length; i++)
		{
			CopyXPs[i].PlayerName = myClient.CurrentXPs[i].PlayerName;
			CopyXPs[i].PlayerClass = myClient.CurrentXPs[i].PlayerClass;
			CopyXPs[i].SubClass = myClient.CurrentXPs[i].SubClass;
			CopyXPs[i].InitialLV = myClient.CurrentXPs[i].InitialLV;
			CopyXPs[i].CurrentXP = myClient.CurrentXPs[i].CurrentXP;
			CopyXPs[i].NeededXP = myClient.CurrentXPs[i].NeededXP;
			CopyXPs[i].XPGained = myClient.CurrentXPs[i].XPGained;
		}
	...
	if (!isSpectator)
	{
		...
		C.SetPos(DX, rowTextY+rowSegmentHeight);
		if (player.bOutOfLives)
			C.DrawText("DEAD");
		else
			C.DrawText("ALIVE");
		...
		C.DrawColor = PrimaryColor;
		C.DrawText(string(L));
		//C.SetPos(LX, rowTextY+rowSegmentHeight);
		//C.DrawText(string(Credits));

		// Draw RPGExp value
		C.SetPos(EX, rowTextY);
		C.DrawColor = PrimaryColor;
		C.DrawText(EXP$"/"$NeedXP);
		C.SetPos(EX, rowTextY+rowSegmentHeight);
		C.DrawText(string(XPGain));

		// Draw RPGClass value
		C.SetPos(CX, rowTextY);
		C.DrawColor = PrimaryColor;
		if (PClass == 0)
			C.DrawText("None");
		else if (PClass == 1)
			C.DrawText("Weapons Master");
		else if (PClass == 2)
			C.DrawText("Adrenaline Master");
		else if (PClass == 3)
			C.DrawText("Medic Master");
		else if (PClass == 4)
			C.DrawText("Engineer Master");
		else if (PClass == 5)
			C.DrawText("General Master");
		// Draw RPGSubClass value
		C.SetPos(CX, rowTextY+rowSegmentHeight);
		if (PClass == 1)
		{
			if (SClass != "" && SClass != "None")
				C.DrawText(SClass);
			else
				C.DrawText("Get lvl: 200");
		}
		else if (PClass == 2)
		{
			if (SClass != "" && SClass != "None")
