...
function GiveTo(Pawn Other, optional Pickup Pickup)
{
	...
	do
	{
		...
		if (RandNavPt == None)
			break;
		if (RandNavPt.bMayCausePain || RandNavPt.Location.Z <= RandNavPt.Region.Zone.KillZ || (RandNavPt.IsA('FlyingPathNode') && !Instigator.bCanFly))
		{
			RandNavPt = None;
			continue;
		}
	...
	} until ((Dist < 15000 && (Dist > 1000 || !FastTrace(RevivePoint, Instigator.Location)) && !FastTrace(RevivePoint - vect(0,0,500), RevivePoint)) || Count == 1000)

	if (RandNavPt == None || ((Dist < 15000 && (Dist > 1000 || !FastTrace(RevivePoint, Instigator.Location)) && !FastTrace(RevivePoint - vect(0,0,500), RevivePoint)))
	     || Count == 1000 || (Other.Physics != PHYS_Walking && Other.Physics != PHYS_Falling && Other.Physics != PHYS_Swimming))
	{
	...
	if (OwnerController != None)
	{
		...
		switch(OwnerAbilityLevel)
		{
			...
			case 3:
				...
				break;
			default:
				Instigator.ReceiveLocalizedMessage(class'GhostMessage', 0);
				Instigator.Health = Instigator.HealthMax;
				break;
		}
