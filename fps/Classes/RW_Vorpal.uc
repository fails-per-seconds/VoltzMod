...
static function bool AllowedFor(class<Weapon> Weapon, Pawn Other)
{
	...
	if (instr(caps(string(Weapon)), "AVRIL") > -1)
		return true;
	...
	if (Damage > 0 && Chance >= rand(99))
	{
		if (Victim != None && Victim.isA('Pawn'))
		{
			...
			if (Rules == None)
				CheckRPGRules();
			if (Rules != None)
				Rules.AwardEXPForDamage(Instigator.Controller, RPGStatsInv(Instigator.FindInventoryType(class'RPGStatsInv')), Pawn(Victim), Pawn(Victim).Health - 1);
