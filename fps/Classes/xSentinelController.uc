class xSentinelController extends ASSentinelController;
	...
	if (TimeSinceCheck > 1.0)
	{
		...
		while (C != None)
		{
			     ...
			     && ((TeamGame(Level.Game) != None && !C.SameTeamAs(PlayerSpawner)) || (TeamGame(Level.Game) == None && C.Pawn.Owner != PlayerSpawner)))
			{
				...
				if (C != None && MonsterController(C) != None && (C.Enemy == PlayerSpawner.Pawn || C.Enemy == None) && FRand() < 0.2 && VSize(C.Pawn.Location - Pawn.Location) < AttractRange)
