class WeaponLinkTurret extends Weapon_Linkturret
	HideDropDown
	CacheExempt
	config(user);
...
function bool ConsumeAmmo(int Mode, float load, optional bool bAmountNeededIsMax)
{
	...
	if (Linking && LinkFire(FireMode[1]).LockedPawn != None && (LinkGun(LinkFire(FireMode[1]).LockedPawn.Weapon) != None
	...
	if (Instigator != None && Instigator.PlayerReplicationInfo != None && Instigator.PlayerReplicationInfo.Team != None)
	{
		for (C = Level.ControllerList; C != None; C = C.NextController)
			if (C.Pawn != None && C.Pawn.Weapon != None)
			{
				...
				else if (RPGWeapon(C.Pawn.Weapon) != None && LinkGun(RPGWeapon(C.Pawn.Weapon).ModifiedWeapon) != None && LinkGun(RPGWeapon(C.Pawn.Weapon).ModifiedWeapon).LinkedTo(self))
...
function bool FocusOnLeader(bool bLeaderFiring)
{
	...
	else
	{
		...
		{
			LeaderPawn = V;
			bLeaderFiring = (LeaderPawn.Health < LeaderPawn.HealthMax) && (V.LinkHealMult > 0) && ((B.Enemy == None) || V.bKeyVehicle);
		}
	}
	if (LeaderPawn == None)
	{
	...
	if ( !bLeaderFiring && (LeaderPawn.Weapon == None || !LeaderPawn.Weapon.IsFiring()) )
		return false;
	if ((Vehicle(LeaderPawn) != None) || ((LinkGun(LeaderPawn.Weapon) != None || (RPGWeapon(LeaderPawn.Weapon) != None && LinkGun(RPGWeapon(LeaderPawn.Weapon).ModifiedWeapon) != None))
...
function float GetAIRating()
{
	...
	if ((PlayerController(B.Squad.SquadLeader) != None) && (B.Squad.SquadLeader.Pawn != None) && (LinkGun(B.Squad.SquadLeader.Pawn.Weapon) != None
	     || (RPGWeapon(B.Squad.SquadLeader.Pawn.Weapon) != None && LinkGun(RPGWeapon(B.Squad.SquadLeader.Pawn.Weapon).ModifiedWeapon) != None)))
		return 1.2;

	V = B.Squad.GetLinkVehicle(B);
	if ((V != None) && (VSize(Instigator.Location - V.Location) < 1.5 * LinkFire(FireMode[1]).TraceRange) && (V.Health < V.HealthMax) && (V.LinkHealMult > 0))
		return 1.2;

	if (Vehicle(B.RouteGoal) != None && B.Enemy == None && VSize(Instigator.Location - B.RouteGoal.Location) < 1.5 * LinkFire(FireMode[1]).TraceRange && Vehicle(B.RouteGoal).TeamLink(B.GetTeamNum()))
		return 1.2;

	O = DestroyableObjective(B.Squad.SquadObjective);
	if (O != None && B.Enemy == None && O.TeamLink(B.GetTeamNum()) && O.Health < O.DamageCapacity && VSize(Instigator.Location - O.Location) < 1.1 * LinkFire(FireMode[1]).TraceRange && B.LineOfSightTo(O))
		return 1.2;
...
function byte BestMode()
{
	...
	if (V == B.Target)
		return 1;
	if ((V != None) && (VSize(Instigator.Location - V.Location) < LinkFire(FireMode[1]).TraceRange) && (V.Health < V.HealthMax) && (V.LinkHealMult > 0) && B.LineOfSightTo(V))
		return 1;
	if (B.Enemy == None)
		return 0;
...
defaultproperties
{
     FireModeClass(0)=Class'fps.FM_LinkFire'
     FireModeClass(1)=Class'fps.FM_LinkAltFire'
}
